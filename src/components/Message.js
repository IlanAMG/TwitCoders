...
import { FiHeart, FiUpload, FiX, FiMessageCircle, FiRefreshCw } from 'react-icons/fi'
...
export const Message = ({ message }) => {
    ...
    useEffect(() => {
        if (user) {
            const isLike = message.likes.some(like => like.likeBy.id === user.uid) //si y'a un user on vérifie pour chaque like pour savoir si ya un like de l'user
            setIsLike(isLike) // dès le chargement on synchronise directement pour savoir les likes de l'user
        }
    }, [user, message.likes])
    ...
    const handleLike = () => {
        ...
        if (!isLike) {
            ...
            const updatedLikes = [...message.likes, like] //ici on copie le tableau likes dans la bdd puis on rajoute notre nouvel objet like
    ...
    return (
        <div className='message-container'>
            <div>
                <img className='profil-picture' src={message.photo} alt='Profil'/>
            </div>
